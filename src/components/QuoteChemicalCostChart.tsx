...
interface ChemicalCostChartProps {
  ...
  compact?: boolean;
  compactMode?: 'tabs' | 'demand-only' | 'total-only';
  dataNote?: string;
  visitsPerMonthLabel?: string;
}
...
const DEFAULT_CHEM_DATA_NOTE = 'Based on real billing data from 2,000+ weekly residential cleaning visits.';

function getMonthlyChartSubscript(visitsPerMonthLabel: string) {
  return visitsPerMonthLabel === '4.33'
    ? 'Includes labor at 4.33 visits/month to account for 5-visit months'
    : `Includes labor at ${visitsPerMonthLabel} visits/month for the selected month scenario`;
}

function CompactChemView({
  combined,
  competitorFlat,
  mode = 'tabs',
  dataNote = DEFAULT_CHEM_DATA_NOTE,
  visitsPerMonthLabel = '4.33',
}: {
  combined: CombinedRow[];
  competitorFlat: number;
  mode?: 'tabs' | 'demand-only' | 'total-only';
  dataNote?: string;
  visitsPerMonthLabel?: string;
}) {
  const [activeTab, setActiveTab] = useState<'demand' | 'total'>('demand');
  const activeView = mode === 'demand-only' ? 'demand' : mode === 'total-only' ? 'total' : activeTab;

  return (
    <div class={`chem-compact chem-compact--${mode}`}>
      {mode === 'tabs' && (
...
export default function ChemicalCostChart({
  data,
  serviceMonthly,
  competitorFlat = 300,
  compact = false,
  ...
  if (compact) {
    return (
      <CompactChemView
        combined={combined}
        competitorFlat={competitorFlat}
        mode={compactMode}
        dataNote={dataNote}
        visitsPerMonthLabel={visitsPerMonthLabel}
      />
    );
  }
  ...
  return (
    <div class="chem-cost-section">
      ...
      <div class="chem-chart-insight">
        Total monthly cost {getMonthlyChartSubscript(visitsPerMonthLabel).toLowerCase()}.
      </div>
    </div>
  );
}
...
function renderChemNumberLine(combined: CombinedRow[]) {
  return <ChemNumberLine combined={combined} />;
}

function getCurrentChemicalSeasonKey(date = new Date()) {
...
function ChemNumberLine({ combined }: { combined: CombinedRow[] }) {
  ...
  const seasons = [
    { label: 'Winter', periodLines: ['Nov-Feb'], ...w, cls: 'winter', color: '#2563eb', callout: 'Lower demand — pool sits cooler, less evaporation' },
    { label: 'Spring/Fall', periodLines: ['Mar-May', 'Sep-Oct'], ...sh, cls: 'shoulder', color: '#059669', callout: 'Pollen season + transitional temps shift demand' },
    { label: 'Summer', periodLines: ['Jun-Aug'], ...s, cls: 'summer', color: '#d97706', callout: 'Peak usage — heat, sun, and swimmers drive demand' },
  ].filter(s => s.high > 0);
  const defaultSeasonIndex = Math.max(0, seasons.findIndex(s => s.cls === getCurrentChemicalSeasonKey()));
  const [activeSeasonIndex, setActiveSeasonIndex] = useState(defaultSeasonIndex);
  const activeSeason = seasons[activeSeasonIndex] || seasons[0];

  if (!activeSeason || !Number.isFinite(range) || range <= 0) return null;

  const nextSeason = () => {
    setActiveSeasonIndex(index => (index + 1) % seasons.length);
  };
  const activeMidPct = activeSeason.high === activeSeason.low
    ? 50
          ...
          <div class="chem-numberline__row">
            ...
            <div class="chem-numberline__track">
              <div
                ...
                style={{ left: `${pct(activeSeason.low)}%`, width: `${pct(activeSeason.high) - pct(activeSeason.low)}%` }}
              >
                <span class="chem-numberline__mid" style={{ left: `${activeMidPct}%` }}>
                  <span class="chem-numberline__mid-label">${activeSeason.mid}</span>
                </span>
              </div>
            </div>
            <span class="chem-numberline__range" style={{ color: activeSeason.color }}>${activeSeason.low}–${activeSeason.high}</span>
          </div>
          <div class="chem-numberline__callout" style={{ color: activeSeason.color }}>{activeSeason.callout}</div>
        </div>
      </div>
...
function renderSeasonalChart(combined: CombinedRow[], competitorFlat: number, visitsPerMonthLabel: string = '4.33') {
  const CHART_W = 480;
  const CHART_H = 220;
  ...
  return (
    <div class="chem-chart-container">
      <div class="chem-chart-title">Total Monthly Cost (Labor + Chemicals)<span class="chem-chart-subscript">{getMonthlyChartSubscript(visitsPerMonthLabel)}</span></div>
